...
- `server/.env.local`에는 `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_WORKER_NAME`, `CLOUDFLARE_API_BASE_URL` 자리를 함께 만들어 두고, `server/package.json`의 `deploy`는 이 파일을 읽어 원격 Worker를 다시 배포합니다.
