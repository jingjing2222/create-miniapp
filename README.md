...
옵션으로 주지 않은 값은 한국어 프롬프트로 이어지며, package manager 선택이 가장 먼저 나옵니다. `server-provider`를 선택하면 provider에 맞는 원격 리소스를 새로 만들지, 기존 것을 쓸지도 이어서 묻습니다. `--yes`를 쓰면 원격 연결은 건너뛰고 로컬 scaffold만 진행합니다.
...
Cloudflare Worker 연결 방식은 두 가지입니다.

- 새 Worker 생성: 스캐폴드 중 Wrangler 로그인과 account 선택을 거쳐 Worker를 배포하고, `workers.dev` URL을 계산할 수 있으면 `frontend/.env.local`과 optional `backoffice/.env.local`까지 자동으로 작성합니다.
- 기존 Worker 사용: account와 Worker를 고른 뒤 같은 URL을 계산할 수 있으면 동일하게 `.env.local`까지 자동으로 작성합니다.
