...
worktree 레이아웃을 골랐다면 실제 repo root는 `my-miniapp/main`이 돼요. control root에는 로컬 안내용 `AGENTS.md`, `README.md`, `.git`, `.bare/`만 남고, 이후 worktree 관리는 `wt` 기준으로 진행해요. 이 CLI는 `wt`를 자동 설치하지 않으니, worktree mode를 쓸 때는 미리 준비해 두는 편이 좋아요.
...
<appName>/
  ...
  main/
    ...
    nx.json
    biome.json
  feat-a/       # `wt add -c feat-a -b main` 예시
```

이 구조에서는 control root에서 `wt status`, `wt add -c <branch> -b main`, `wt pull`, `wt remove <branch> -b` 같은 명령으로 worktree를 관리하고, 실제 코드 수정과 `pnpm verify`는 `main/`이나 추가 worktree 안에서 진행하면 돼요.
