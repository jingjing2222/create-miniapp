...
이 문서는 **MiniApp 구현 시 바로 참조할 수 있는 안정적인 API 가이드**입니다.

- 마지막 업데이트: 2026-03-14
- 원칙:
  - 로컬 경로(`node_modules`, 임시 생성 JSON, 내부 스캔 결과)에 의존하지 않는다.
  - 공식 공개 문서만 소스 오브 트루스로 사용한다.
  - 구현자는 본 문서에서 후보 API를 찾고, 링크된 원문에서 시그니처/제약을 최종 확인한다.
- 네이티브 모듈 사용 제약은 `./native-modules-policy.md`를 따른다.

---

## 1) Stable Official Sources
...
## 2) API Discovery Flow (실무용)

1. 기능 요구사항을 분류한다: `화면 이동 / 환경 확인 / 권한 / 네트워크 / 저장소 / 위치 / 공유 / 분석 / 광고 / 결제`.
...
3. 반드시 원문 링크에서 아래를 재확인한다.
   ...
   - 권한 요구사항
   - 에러 타입/예외 처리 방식
4. 구현 후 테스트 케이스에 “권한 거부/미지원 환경” 시나리오를 포함한다.
...
- 최신 React Native 계열 버전에 맞는 런타임, 타입 기반으로 움직인다
- 의존성 구조: Granite 패키지/React 계열 버전군 재정렬

### 3.2 API 관점 정리
- 공식 SDK 2.0.1 가이드에는 **함수별 SDK API 시그니처의 명시적 breaking list**가 제공되지 않는다.
- 따라서 실무에서는 다음을 API 변경 리스크로 본다:
  1. RN 0.84/React 19 전환으로 인한 타입/런타임 동작 차이
  2. 빌드 체인 전환(`ait build`)에 따른 실행 환경 차이
  3. 패키지 재배치로 인한 import/export 경로 영향

### 3.3 React Native 프로젝트 적용 절차
1. `@apps-in-toss/framework`를 먼저 설치
2. `ait init`로 AppInToss 설정을 반영
3. `ait build`로 빌드 검증
4. 타입 오류/런타임 경고 수동 보정
5. 최신 샌드박스 앱으로 검증
...
- 온보딩/약관 닫기 버튼 → `closeView`
- 작성 중 이탈 방지 모달 → `useBackEvent`

## 4.2 Environment / Device
...
- 러닝/배달 추적 화면 → `startUpdateLocation` 또는 `useGeolocation`

## 4.5 Network / Storage / Clipboard
...
## 5) Granite-Side Implementation Notes

- Granite API는 항상 공식 레퍼런스(react-native) 기준으로 적용한다.
- 라우팅 경로는 MiniApp 규칙에 맞는 **고정 경로 기준**으로 관리한다.
- 플랫폼별 API 차이는 분기 처리하고, 에러 타입을 명시적으로 핸들링한다.

### Granite 실무 포인트 (이번 보강)
- `defineConfig`: 스킴/앱이름/플러그인/번들러 설정의 단일 진입점
  - https://www.granite.run/reference/react-native/config/defineConfig.html
- `useVisibilityChange`: 화면 visible/hidden 전환 타이밍에서 로깅/정리 로직 처리
  - https://www.granite.run/ko/reference/react-native/screen-control/useVisibilityChange.html
- `Video`: 오디오 포커스 변경(onAudioFocusChanged) 대응이 필요한 영상 재생 화면에서 사용
...
참고: https://www.granite.run/ko/reference/react-native/
