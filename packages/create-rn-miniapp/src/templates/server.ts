...
function renderSupabaseInstallDenoScript() {
  return [
    ...
    '',
    '  if (installedCommand) {',
    "    console.log('[server] Deno를 찾았어요. 최신 버전으로 `deno upgrade`를 실행할게요.')",
    "    runCommand(installedCommand, ['upgrade'])",
    '  } else if (process.platform === "win32") {',
    "    console.log('[server] Deno가 없어서 Windows installer로 최신 버전을 설치할게요.')",
    ...
    '    )',
    '  } else {',
    "    console.log('[server] Deno가 없어서 install.sh로 최신 버전을 설치할게요.')",
